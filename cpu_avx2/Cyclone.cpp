...
static constexpr double STATUS_INTERVAL_SEC     = 1.0;
...
static std::vector<char>            g_threadDone;
static std::string                  g_threadRemainReport;
static bool                         g_saveCandidates    = false;
static std::string                  g_statsFile;
...
static void writeStatsFile(int threads,
                           double mkeys,
                           ...
                           bool found)
{
    if (g_statsFile.empty()) return;

    std::ofstream ofs(g_statsFile, std::ios::trunc);
    if (!ofs) return;
    ofs << "CPU_THREADS=" << threads << "\n";
    ofs << "CPU_MKEYS=" << std::fixed << std::setprecision(3) << mkeys << "\n";
    ofs << "CPU_CHECKED=" << checked << "\n";
    ofs << "CPU_ELAPSED=" << std::fixed << std::setprecision(3) << elapsed << "\n";
    ofs << "CPU_PROGRESS=" << std::fixed << std::setprecision(6) << (double)progress << "\n";
    ofs << "CPU_DONE=" << (done ? 1 : 0) << "\n";
    ofs << "CPU_FOUND=" << (found ? 1 : 0) << "\n";
    ofs << g_threadRemainReport;
}
                                ...
                                const std::vector<uint64_t>& b)
{
    ...
    for (size_t i = 0, sz = std::max(a.size(), b.size()); i < sz; ++i) {
        ...
        uint64_t xy = x + y;
        uint64_t carry1 = (xy < x) ? 1ULL : 0ULL;
        uint64_t sum = xy + carry;
        uint64_t carry2 = (sum < xy) ? 1ULL : 0ULL;
        carry = carry1 | carry2;
        s.push_back(sum);
    }
...
static void printUsage(const char* prog)
{
    std::cerr<<"Usage: "<<prog
             ...
             <<" [--bench-seconds N] [--stats-file PATH] [--quiet]\n";
}
...
// Report what each CPU thread has left, so a GPU that finishes its own share
// early can take the leftovers instead of idling until the sidecar catches up.
// Each thread owns one contiguous chunk and walks it in order, so the leftover
// is [current key, chunk end]. g_threadPrivateKeys[] holds the start of the
// batch in flight, which makes this conservative by at most one batch -- the
// GPU may rescan a few keys, but it can never be handed a gap.
static void buildThreadRemainReport(int numCPUs)
{
    // Int::GetBase16() yields uppercase while bigNumToHex() yields lowercase;
    // fold both before comparing, or the fixed-width compare below always sees
    // 'A'(65) < 'a'(97) and clamps every thread back to its chunk start.
    auto lower = [](std::string h) {
        for (size_t i = 0; i < h.size(); ++i)
            if (h[i] >= 'A' && h[i] <= 'F') h[i] = (char)(h[i] - 'A' + 'a');
        return h;
    };
    std::ostringstream oss;
    for (int i = 0; i < numCPUs; ++i) {
        if (i >= (int)g_threadRanges.size()) break;
        if (i < (int)g_threadDone.size() && g_threadDone[i]) {
            oss << "CPU_T" << i << "=DONE\n";
            continue;
        }
        const std::string beg = lower(padHexTo64(g_threadRanges[i].startHex));
        const std::string end = lower(padHexTo64(g_threadRanges[i].endHex));
        std::string cur = (i < (int)g_threadPrivateKeys.size())
                            ? lower(padHexTo64(g_threadPrivateKeys[i])) : std::string();
        // Zero-padded to a fixed 64 chars, so lexicographic order is numeric order.
        if (cur.size() != 64 || cur < beg) cur = beg;
        if (cur > end) { oss << "CPU_T" << i << "=DONE\n"; continue; }
        oss << "CPU_T" << i << "=" << cur << ":" << end << "\n";
    }
    g_threadRemainReport = oss.str();
}

#if defined(CUDACYCLONE_EMBEDDED_CPU_WORKER)
int cpu_avx2_worker_main(int argc, char* argv[])
#else
...
#endif
{
    ...
    for(int i=1;i<argc;++i){
        ...
        else if(!std::strcmp(argv[i],"--bench-seconds") && i+1<argc){
            benchSeconds=std::stoi(argv[++i]);
            if(benchSeconds<1||benchSeconds>3600){
                std::cerr<<"--bench-seconds must be 1-3600\n"; return 1;
            }
            benchmarkMode=true;
        }
        else if(!std::strcmp(argv[i],"--stats-file") && i+1<argc){
            g_statsFile=argv[++i];
        }
    ...
    g_threadDone.assign(numCPUs,(char)0);
...
#pragma omp critical(full_match)
                            ...
                            // MSVC's OpenMP does not support cancel; other threads exit via matchFound.
                        }
                        ++localChecked;
                    }
                    localCnt=0;
...
#pragma omp critical
                {
                    ...
                    buildThreadRemainReport(numCPUs);
                    writeStatsFile(numCPUs,mkeys,globalChecked,globalElapsed,prog,false,false);
                    lastStat=now;
                }
            }
            ...
            {
#pragma omp critical(bench_stop)
                {
                    timeExpired=true;
                }
            }
...
#pragma omp critical
        ...
        // Only a thread that ran past its own end is finished; one stopped by a
        // match or the time limit still has work left to hand over.
        if (intGreater(priv, privEnd)) {
#pragma omp critical
            g_threadDone[tid] = (char)1;
        }
...
#pragma omp atomic
    ...
    if(benchmarkMode){
        ...
        double benchMkeys=(elapsed>0.0) ? (double)globalChecked / elapsed / 1e6 : 0.0;
        buildThreadRemainReport(numCPUs);
        writeStatsFile(numCPUs,benchMkeys,globalChecked,elapsed,0.0L,true,false);
        std::cout<<"\nCPU_BENCH_THREADS="<<numCPUs<<"\n"
                 <<"CPU_BENCH_SECONDS="<<std::fixed<<std::setprecision(3)<<elapsed<<"\n"
                 <<"CPU_BENCH_KEYS="<<globalChecked<<"\n"
                 <<"CPU_BENCH_MKEYS="<<std::fixed<<std::setprecision(3)<<benchMkeys<<"\n";
        return 0;
    }
    ...
    if(!matchFound){
        auto tEnd=std::chrono::high_resolution_clock::now();
        double elapsed=std::chrono::duration<double>(tEnd - tStart).count();
        double finalMkeys=(elapsed>0.0) ? (double)globalChecked / elapsed / 1e6 : 0.0;
        buildThreadRemainReport(numCPUs);
        writeStatsFile(numCPUs,finalMkeys,globalChecked,elapsed,100.0L,true,false);
        std::cout<<"\nNo match found.\n";
        return 0;
    }
    writeFoundKey(foundPriv, foundPub, foundWIF, targetAddress);
    {
        ...
        long double prog = totalRangeLD>0.0L
            ...
            : 0.0L;
        buildThreadRemainReport(numCPUs);
        writeStatsFile(numCPUs,finalMkeys,globalChecked,elapsed,prog,true,true);
    }
