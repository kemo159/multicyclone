...
__device__ __forceinline__ uint32_t hashBswap32(uint32_t x){
...
__device__ __forceinline__ void SHA256_33_from_limbs(uint8_t prefix02_03, const uint64_t x_be_limbs[4], uint32_t out_state[8]){
    ...
    uint32_t w[16];
    // NB: do not "optimise" this into explicit __byte_perm calls. Every
    // extraction here is byte-aligned, so PRMT looks like an obvious win, but
    // nvcc already canonicalises this form to the same SASS: a 9-word
    // __byte_perm rewrite was measured byte-identical (1384 instructions,
    // PRMT=3, SHF=647, LOP3=347 either way) and bit-identical over 8.4M
    // vectors. It buys nothing and costs readability.
                                                            ...
                                                            uint8_t ripemd20[20])
{
    uint32_t W[16];
#pragma unroll
    for(int i=0;i<8;++i) W[i] = hashBswap32(sha_state_be[i]);
    W[8]  = 0x00000080u;
#pragma unroll
...
__device__ __forceinline__ uint32_t RIPEMD160_prefix32_from_SHA256_state(
    const uint32_t sha_state_be[8])
{
    uint32_t W[16];
#pragma unroll
    for (int i = 0; i < 8; ++i) W[i] = hashBswap32(sha_state_be[i]);
...
__device__ __forceinline__ uint32_t getHash160Prefix32_33_from_limbs(
    uint8_t prefix02_03,
    const uint64_t x_be_limbs[4])
{
    uint32_t sha_state[8];
    SHA256_33_from_limbs(prefix02_03, x_be_limbs, sha_state);
    return RIPEMD160_prefix32_from_SHA256_state(sha_state);
}

__device__ __forceinline__ uint32_t SHA256_33_diag_from_limbs(uint8_t prefix02_03,
                                                           const uint64_t x_be_limbs[4])
{
    uint32_t sha_state[8];
    SHA256_33_from_limbs(prefix02_03, x_be_limbs, sha_state);
...
__device__ __forceinline__ uint32_t RIPEMD160_diag_from_limbs(const uint64_t x_be_limbs[4])
{
    uint32_t sha_state[8];
...
// Keep the astronomically rare full verification out of the fused search hot
// path.  If this is force-inlined beside getHash160Prefix32_33_from_limbs(),
// ptxas can common-subexpression-eliminate both hashes into one full HASH160
// and predicate all 20 target-byte comparisons for every candidate.
__device__ __noinline__ bool verifyHash160_33_from_limbs_rare(
    uint8_t prefix02_03,
    const uint64_t x_be_limbs[4],
    const uint8_t target20[20])
{
    uint8_t hash20[20];
    getHash160_33_from_limbs(prefix02_03, x_be_limbs, hash20);
#pragma unroll
    for (int i = 0; i < 20; ++i) {
        if (hash20[i] != target20[i]) return false;
    }
    return true;
}
