...
- **--partial**: save candidates whose hash160 starts with the requested number of target hex digits. Exact-length matches go to `partial.txt`; longer matches go to `partialpN.txt` where `N` is the extra matching hex digits.
...
With `--partial 6`, a 6-hex-character hash160 prefix match is saved to `partial.txt`. A 9-character prefix match is saved to `partialp3.txt`.
...
```
## 🛠️ Getting Started
CUDA 13.0 or newer is recommended. The project supports native Windows and Linux builds.

For full dependency and build setup instructions, see [BUILD.md](BUILD.md).
...
Use `-DCMAKE_CUDA_ARCHITECTURES=89` for RTX 40xx, `90` for Hopper, or `120`/`121` for Blackwell-generation GPUs. You can also pass a semicolon-separated list, for example `-DCMAKE_CUDA_ARCHITECTURES="89;120"`.
